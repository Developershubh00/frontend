...
interface ClosingRanks2025UGPageProps {
  onBack: () => void;
}

// Updated Interface to match UG CSV structure
interface RanksData {
  Quota: string;
  Category: string;
  State: string;
...
// ── Main component ─────────────────────────────────────────────────────────
const ClosingRanks2025UGPage: React.FC<ClosingRanks2025UGPageProps> = ({
  onBack,
}) => {
  ...
  const [selRound, setSelRound] = useState<"all" | "1" | "2" | "3">("all");
  ...
  const applyRound = (round: "all" | "1" | "2" | "3") => {
    setSelRound(round);
    const nv = { ...colVis };
    const allRankCols: ColKey[] = ["CR 2025 1", "CR 2025 2", "CR 2025 3"];
  ...
  useEffect(() => {
    // Ensure your file is named ClosingRanks2025UG.csv and placed in public/data/
    fetch("/data/ClosingRanks2025UG.csv")
      .then((r) => {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((item) => {
      ...
      if (minRank || maxRank) {
        const rankCols: ColKey[] = ["CR 2025 1", "CR 2025 2", "CR 2025 3"];
            ...
            <div className="p-4">
              ...
              {[
                ...
                },
                {
                  ...
                  keys: ["CR 2025 1", "CR 2025 2", "CR 2025 3"] as ColKey[],
                },
              ].map((group) => (
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-lg font-semibold">UG Closing Ranks</h1>
        ...
        {dataError && (
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-2 text-xs text-amber-700 text-center">
            ...
            <code>/data/ClosingRanks2025UG.csv</code> to enable this page.
          </div>
        )}

        {/* ── Round pills & Categories ── */}
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {paged.length === 0 ? (
                <tr>
                  ...
                  >
                    {dataError
                      ? "CSV not found — add /data/ClosingRanks2025UG.csv to enable."
                      ...
                      <td className="px-2 py-2 text-center text-xs">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            item.Category === "GEN" || item.Category === "Open"
...
export default ClosingRanks2025UGPage;
