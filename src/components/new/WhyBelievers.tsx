import { BarChart3, BrainCircuit, Database, GraduationCap, MapPinned, ShieldCheck } from 'lucide-react'
    ...
    <section className="wb-section" aria-labelledby="why-believers-title">
      <div className="wb-container">
        <header className="wb-header">
          <p className="wb-eyebrow"><span className="wb-eyebrow-line" />WHY BELIEVERS</p>
          <h1 className="wb-title" id="why-believers-title">Medical Career Decisions Need More Than Data.</h1>
        </header>
              ...
              {guidancePoints.map(({ label, icon: Icon }, index) => (
                <article className="wb-card" key={label}>
                  <div className="wb-card-top">
                    <span className="wb-card-number">{String(index + 1).padStart(2, '0')}</span>
                    <span className="wb-icon" aria-hidden="true"><Icon size={22} strokeWidth={1.8} /></span>
                  </div>
                  <div className="wb-card-content"><h3>{label}</h3></div>
        ...
        <div className="wb-cta">
          <div className="wb-cta-copy"><p>Data can show you the options.</p><h2>Guidance helps you choose.</h2></div>
          <a className="wb-button" href="#contact">Talk to a Counsellor</a>
        </div>
      </div>
    </section>
  )
