 ...
 *   /public/data/neetPgData/neetpg_2025_closing_rank.csv
...
const CRCard: React.FC<{ row: ClosingRankResult }> = ({ row }) => {
  ...
  const bar: Record<Chance, string> = {
    "Very High": "92%",
    High: "68%",
    Moderate: "42%",
    Low: "18%",
  ...
  return (
    <div
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.boxShadow = "0 4px 18px rgba(0,0,0,.13)")
      }
      onMouseLeave={(e) =>
    ...
    >
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            <Pill bg="#eef2ff" color="#4338ca">
              {row["Quota"]} · {row["Category"]}
            </Pill>
      ...
      >
        <div
          style={{
            width: row.chance ? bar[row.chance] : "5%",
            height: "100%",
            background: c.bar,
            borderRadius: 999,
            transition: "width .6s",
      ...
      {(row["Fee"] || row["Stipend Year 1"] || row["Beds"]) && (
        ...
        >
          ...
          {row["Stipend Year 1"] && (
            <Detail label="Stipend" value={`₹${row["Stipend Year 1"]}/yr`} />
          )}
          {row["Bond Years"] && row["Bond Years"] !== "0" && (
            <Detail label="Bond" value={`${row["Bond Years"]} yr`} />
          )}
  ...
  >
    ...
    >
      <div style={{ flex: 1 }}>
        ...
        <p
          style={{ margin: 0, color: "#6366f1", fontWeight: 600, fontSize: 13 }}
        >
          {SPECIALTY_LABELS[row["Course"]] || row["Course"]}
        </p>
      </div>
...
const SeatCardItem: React.FC<{ row: CSVRow }> = ({ row }) => (
  ...
  >
    <div>
      ...
      <Pill bg="#f3f4f6" color="#374151">
        {row["State"]} · {row["Management of college"]}
      </Pill>
    </div>
    <div
      style={{
...
const Results: React.FC<ResultsProps> = ({
  ...
  const displayed = results.filter((r) => {
    ...
    if (
      mode === "closing" &&
      cf !== "ALL" &&
      (r as ClosingRankResult).chance !== cf
    )
      return false;
    return true;
  });
  ...
  const summary =
    mode === "closing"
      ? (["Very High", "High", "Moderate", "Low"] as Chance[]).reduce(
          (acc, ch) => ({
            ...acc,
            [ch]: results.filter((r) => (r as ClosingRankResult).chance === ch)
              .length,
          }),
  ...
  return (
    ...
    >
      ...
      {summary && results.length > 0 && (
        ...
        >
          {(
            ...
            <button
              key={key}
              onClick={() => setCf(key as Chance | "ALL")}
              style={{
      ...
      {displayed.length === 0 && (
        ...
        >
          ...
          <p style={{ fontSize: 13, margin: 0 }}>
            Try a different quota, category, or widen your specialty / state
            selection.
          </p>
        </div>
      )}
      ...
      >
        {mode === "closing" &&
          displayed.map((r, i) => (
            <CRCard key={i} row={r as ClosingRankResult} />
          ))}
        {mode === "allotments" &&
          displayed.map((r, i) => <AllotCard key={i} row={r as CSVRow} />)}
        {mode === "seats" &&
          displayed.map((r, i) => <SeatCardItem key={i} row={r as CSVRow} />)}
      </div>
    </div>
  );
};
...
const Hero: React.FC<{ onOpen: () => void }> = ({ onOpen }) => (
  ...
  >
      ...
      {(
        ...
        <div
          ...
          style={{
            ...
            borderRadius: "50%",
            filter: "blur(60px)",
            animation: "_blob 6s ease-in-out infinite",
            animationDelay: b.d,
          }}
        />
      ))}
    ...
    >
      ...
      >
        Predict Your <span style={{ color: "#a5b4fc" }}>Dream College</span>
        <br />
      ...
      >
        Instantly discover which colleges &amp; specialties you can get — based
        on 2025 actual allotment &amp; closing rank data from 28,000+ records.
      </p>
      <div
        style={{
          display: "flex",
      ...
      <p style={{ color: "#818cf8", fontSize: 12, marginTop: 12 }}>
...
const CollegePredictor: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const allotR = parseCSV(allot);
      const seatR = parseCSV(seat);
      setCrData(crR);
      setAllotData(allotR);
      setSeatData(seatR);
      setSpecialties(
      ...
      res = crData
        ...
        .map((row): ClosingRankResult => {
          ...
          return {
            ...
            dataYear: cr25 ? "2025" : cr24 ? "2024" : null,
          };
        })
        .filter((row) => row.chance !== null)
        .sort((a, b) => {
          const ord: Record<Chance, number> = {
            "Very High": 0,
            High: 1,
            Moderate: 2,
            Low: 3,
  ...
  const seatSpecialties = [...new Set(seatData.map((r) => r["Course Name"]))]
    .filter(Boolean)
    .sort();
  const seatStates = [...new Set(seatData.map((r) => r["State"]))]
    .filter(Boolean)
    .sort();
          ...
          >
            ...
            >
              <div>
                ...
                <p
                  style={{ color: "#a5b4fc", fontSize: 13, margin: "4px 0 0" }}
                >
              ...
              {step === 1 && (
                <>
                  ...
                  >
                    ...
                    <FormField label="Specialty / Course">
                      ...
                      >
                        ...
                        {(mode === "seats" ? seatSpecialties : specialties).map(
                          (s) => (
                  ...
                  >
                    ...
                    >
                      {mode === "closing" &&
                        "We compare your rank against actual 2025 closing ranks (rounds 1–3). Falls back to 2024 when 2025 data is unavailable."}
                      {mode === "allotments" &&
                        "Shows real allotments made to candidates within ±30% of your rank across all 2025 rounds."}
                      {mode === "seats" &&
                        "Browse total seats available by specialty and state from the official 2025 seat matrix."}
                    </p>
                  </div>
