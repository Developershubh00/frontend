...
//               </div>
//               <div>
//                 <h1 className="font-bold text-[clamp(14px,2vw,14px)] truncate bg-gradient-to-r from-blue-500 to-blue-900 bg-clip-text text-transparent">
//                   Believers Consultancy
//                 </h1>
//                 <p className="text-[clamp(10px,1vw,10px)] truncate text-slate-500">
...
//   const handleCounsellingFormClick = () => {
//     window.open(
...
const Header: React.FC<HeaderProps> = ({
  ...
  const [searchValue, setSearchValue] = useState("");
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
  const [currentSection, setCurrentSection] = useState<string>("NEET PG");
                    ...
                    {neetOptions.map((option) => (
                      <button
                        ...
                        onClick={() => {
                          navigate(option.path);
                          setActiveDropdown(null);
                          setCurrentSection(option.label);
                        }}
                        className={`w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors ${
                          currentSection === option.label ? "bg-blue-50" : ""
                        }`}
                      >
                        <div className="font-medium text-slate-800">
                          {option.label}
