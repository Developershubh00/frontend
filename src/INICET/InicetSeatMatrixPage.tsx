...
const InicetSeatMatrixPage: React.FC = () => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState("seat-matrix");
          ...
          <div className="p-4 lg:p-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-6 lg:p-8 border border-white/20">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                  <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">INICET Seat Matrix</h1>
                  <p className="text-slate-600">Available seats across INICET institutes</p>
                </div>
              </div>
              <div className="bg-blue-50 rounded-xl p-6 text-center">
                <p className="text-slate-700">Seat matrix data will be displayed here. Connect to your data source to populate this page.</p>
              </div>
            </div>
          </div>
        </div>
