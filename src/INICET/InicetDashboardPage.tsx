...
/**
 * INICET Dashboard Page Component
 * Main dashboard for INICET (AIIMS PG) with sidebar navigation and content area
 * Layout matches NEET dashboard but displays INICET-specific content
 */
const InicetDashboardPage: React.FC = () => {
  const { user } = useAuth();
  const [activeSection, setActiveSection] = useState("inicet");
  ...
  /**
   * Fetch INICET dashboard data on component mount
   * Note: Adjust API endpoints based on your actual INICET API structure
   */
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // Replace with actual INICET API endpoints when available
        ...
        setDashboardData({
          inicetStats: [], // Add INICET-specific stats API when available
          timeline: timeline.data,
          choiceLists: choiceLists.data,
        });
      } catch (error) {
        console.error("Failed to fetch INICET dashboard data:", error);
      }
    };
...
export default InicetDashboardPage;
