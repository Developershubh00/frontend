...
const CM: Record<Chance, { bg: string; border: string; text: string; dot: string; bar: string; light: string }> = {
  "Very High": { bg: "#dcfce7", border: "#86efac", text: "#14532d", dot: "#16a34a", bar: "#22c55e", light: "#f0fdf4" },
  "High":      { bg: "#dbeafe", border: "#93c5fd", text: "#1e3a8a", dot: "#2563eb", bar: "#3b82f6", light: "#eff6ff" },
  "Moderate":  { bg: "#fef9c3", border: "#fde047", text: "#713f12", dot: "#ca8a04", bar: "#eab308", light: "#fefce8" },
  "Low":       { bg: "#fee2e2", border: "#fca5a5", text: "#7f1d1d", dot: "#dc2626", bar: "#ef4444", light: "#fef2f2" },
};
...
const FL: React.FC<{ label: string; hint?: string; children: React.ReactNode }> = ({ label, hint, children }) => (
  <div style={{ marginBottom: 11 }}>
    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
      <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", letterSpacing: 0.5, textTransform: "uppercase" }}>{label}</span>
...
const Stat: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div>
    <div style={{ fontSize: 9, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4 }}>{label}</div>
    <div style={{ fontSize: 11, fontWeight: 700, color: "#334155" }}>{value}</div>
  </div>
);
...
// Closing Rank card
const CRCard: React.FC<{ row: CRRow; idx: number; listView: boolean }> = ({ row, idx, listView }) => {
  const m = row.chance ? CM[row.chance] : CM.Low;
  const pctMap: Record<Chance, number> = { "Very High": 90, High: 65, Moderate: 42, Low: 18 };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 3, flex: listView ? 1 : "unset" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            <Tag bg="#eef2ff" color="#4338ca">{row["Quota"]}</Tag>
            <Tag bg="#f1f5f9" color="#475569">{row["Category"]}</Tag>
            {row["State"] && <Tag bg="#f8fafc" color="#64748b">{row["State"]}</Tag>}
            {row.dataYear && (
              <Tag
        ...
        <div style={{ flexShrink: 0, textAlign: "right" }}>
          <div style={{
            ...
            color: m.text, fontWeight: 800, fontSize: 11,
      ...
      {/* Progress bar */}
      {!listView && (
        <div style={{ background: "#f1f5f9", borderRadius: 99, height: 4, overflow: "hidden", marginBottom: 8 }}>
          <div style={{ width: `${pct}%`, height: "100%", background: m.bar, borderRadius: 99 }} />
        </div>
      )}
      ...
      {!listView && (
        <div style={{
          ...
          {row["Stipend Year 1"] && <Stat label="Stipend" value={`₹${row["Stipend Year 1"]}`} />}
          {row["Bond Years"] && row["Bond Years"] !== "0" && <Stat label="Bond" value={`${row["Bond Years"]}yr`} />}
...
// Allotment card
const AllotCard: React.FC<{ row: CSVRow; idx: number; listView: boolean }> = ({ row, idx, listView }) => (
  <div
    style={{
      background: "#fff", border: "1.5px solid #e2e8f0",
      borderTop: "3px solid #6366f1", borderRadius: 12,
      padding: "13px 13px 11px",
      boxShadow: "0 1px 4px rgba(0,0,0,0.06)",
      transition: "transform .18s, box-shadow .18s",
      animation: `cIn .22s ease both`,
    ...
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{
          ...
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          marginBottom: 7,
        }}>{SL[row["Course"]] || row["Course"] || "—"}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          <Tag bg="#eef2ff" color="#4338ca">Rnd {row["Round"]}</Tag>
          <Tag bg="#f1f5f9" color="#475569">{row["Quota"]}</Tag>
          <Tag bg="#f1f5f9" color="#475569">{row["Category"]}</Tag>
          {row["State"] && <Tag bg="#f8fafc" color="#64748b">{row["State"]}</Tag>}
        </div>
      </div>
      <div style={{ flexShrink: 0, textAlign: "center" }}>
        <div style={{
          ...
          <div style={{ fontSize: 14, fontWeight: 900, color: "#14532d", lineHeight: 1 }}>
            #{parseInt(row["AI Rank"] || "0").toLocaleString()}
          </div>
          <div style={{ fontSize: 9, color: "#16a34a", fontWeight: 700, letterSpacing: 0.3 }}>RANK</div>
        </div>
      </div>
    </div>
    {!listView && (
      <div style={{ display: "flex", gap: 12, borderTop: "1px solid #f1f5f9", paddingTop: 8, marginTop: 8 }}>
        {row["Fee"] && <Stat label="Fee" value={`₹${row["Fee"]}`} />}
        {row["Stipend Year 1"] && <Stat label="Stipend" value={`₹${row["Stipend Year 1"]}`} />}
        {row["Bond Years"] && row["Bond Years"] !== "0" && <Stat label="Bond" value={`${row["Bond Years"]}yr`} />}
      </div>
...
const SeatCard: React.FC<{ row: CSVRow; idx: number }> = ({ row, idx }) => (
  <div
    style={{
      background: "#fff", border: "1.5px solid #e2e8f0",
      borderTop: "3px solid #8b5cf6", borderRadius: 12,
      padding: "13px 13px 11px",
      boxShadow: "0 1px 4px rgba(0,0,0,0.06)",
      transition: "transform .18s, box-shadow .18s",
      animation: `cIn .22s ease both`,
    ...
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          marginBottom: 2,
        }}>{row["College Name"] || "—"}</div>
        <div style={{
          color: "#7c3aed", fontWeight: 700, fontSize: 12,
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          marginBottom: 8,
        }}>{SL[row["Course Name"]] || row["Course Name"] || "—"}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          <Tag bg="#f5f3ff" color="#5b21b6">{row["State"]}</Tag>
          <Tag bg="#f1f5f9" color="#475569">{row["Management of college"]}</Tag>
        </div>
      </div>
      <div style={{ flexShrink: 0, textAlign: "center" }}>
        <div style={{
          ...
          <div style={{ fontSize: 20, fontWeight: 900, color: "#4c1d95", lineHeight: 1 }}>{row["Seats"]}</div>
          <div style={{ fontSize: 9, color: "#7c3aed", fontWeight: 700, letterSpacing: 0.3 }}>SEATS</div>
        </div>
      </div>
    </div>
  </div>
      ...
      {(["Very High", "High", "Moderate", "Low"] as Chance[]).map(c => {
        ...
        return (
          <div key={c} style={{
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: m.dot, lineHeight: 1 }}>{n}</div>
            <div style={{ fontSize: 11, fontWeight: 700, color: m.text, marginTop: 2 }}>{c}</div>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 7, padding: "5px 13px", color: "#c7d2fe",
            cursor: "pointer", fontSize: 13, fontWeight: 600,
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden", minHeight: 0 }}>
        ...
        {/* ══ LEFT PANEL ══ */}
        <div style={{
          width: 288, minWidth: 260, flexShrink: 0,
          background: "#fff",
          borderRight: "1px solid #e2e8f0",
          display: "flex", flexDirection: "column",
          overflowY: "auto",
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 800, color: "#1e1b4b" }}>🔍 Search Filters</div>
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>Set your criteria and predict</div>
          </div>
          ...
          <div style={{ padding: "14px 15px 0", flex: 1 }}>
            ...
            <FL label="Mode">
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {([
                  { id: "closing"    as Mode, icon: "🏥", title: "Closing Ranks",  sub: "Which colleges can I get?" },
                  { id: "allotments" as Mode, icon: "📋", title: "Past Allotments",sub: "Who got what near my rank?" },
                  { id: "seats"      as Mode, icon: "💺", title: "Seat Matrix",    sub: "Available seats by specialty" },
                ]).map(m => (
                  <button key={m.id} onClick={() => { setMode(m.id); setSearched(false); setResults([]); }} style={{
                    ...
                    {mode === m.id && <span style={{ color: "#6366f1", fontSize: 13, flexShrink: 0 }}>✓</span>}
                  </button>
                ))}
              </div>
            </FL>
            ...
            <div style={{
              ...
              <p style={{ margin: 0, color: "#166534", fontSize: 11, lineHeight: 1.5 }}>
                {mode === "closing"    && "2025 round data used; falls back to 2024 if unavailable."}
                {mode === "allotments" && "Shows allotments within ±30% of your rank."}
          ...
          <div style={{
            ...
            {dataError && (
              ...
              }}>⚠️ CSV files not found. Check /public/data/</div>
            )}
            <button
              onClick={predict}
              ...
              style={{
                ...
                boxShadow: canPredict ? "0 4px 14px rgba(99,102,241,0.35)" : "none",
                transition: "all .2s",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
              }}
            >
              {searching ? "⏳ Searching…" : loading ? "⏳ Loading…" : "🔍 Predict My Colleges"}
            </button>
            {searched && (
              <button onClick={() => {
                setResults([]); setSearched(false); setRank(""); setCategory("GEN");
                setQuota("AIQ"); setSpecialty("ALL"); setStateVal("ALL");
              }} style={{
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          {!searched && (
            <div style={{
              ...
              <p style={{ margin: "0 0 24px", color: "#64748b", fontSize: 13, textAlign: "center", maxWidth: 340, lineHeight: 1.6 }}>
                Choose a mode, set your filters on the left, then click <b>Predict My Colleges</b>.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, maxWidth: 440, width: "100%" }}>
                {([
                  { icon: "🏥", title: "Closing Ranks",  desc: "See which colleges you qualify for", m: "closing" as Mode },
                  { icon: "📋", title: "Allotments",     desc: "Real allotments near your rank",     m: "allotments" as Mode },
                  { icon: "💺", title: "Seat Matrix",    desc: "Available seats by specialty",       m: "seats" as Mode },
                ]).map(c => (
                  <div key={c.m} onClick={() => setMode(c.m)} style={{
                    background: "#fff",
                    border: mode === c.m ? "2px solid #6366f1" : "1.5px solid #e2e8f0",
                    borderRadius: 11, padding: "13px 10px", cursor: "pointer", textAlign: "center",
                    transition: "all .15s",
                    boxShadow: mode === c.m ? "0 4px 14px rgba(99,102,241,0.15)" : "0 1px 3px rgba(0,0,0,0.05)",
                  }}>
              ...
              <div style={{ display: "flex", gap: 22, marginTop: 32, flexWrap: "wrap", justifyContent: "center" }}>
          ...
          {searched && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", padding: "16px 18px 0" }}>
                ...
                <div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: "#1e1b4b" }}>
                    ...
                    {shown.length !== results.length && (
                      <span style={{ fontSize: 13, color: "#94a3b8", fontWeight: 500 }}> · showing {shown.length}</span>
                    )}
                  </div>
                  {rank && mode !== "seats" && (
              ...
              {results.length > 0 && (
                <div style={{
                  ...
                  <input
                    ...
                    style={{ ...inp, flex: "1 1 180px", fontSize: 12, padding: "7px 12px" }}
                  />
                  {mode === "closing" && (
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                      {(["ALL", "Very High", "High", "Moderate", "Low"] as (Chance | "ALL")[]).map(key => {
                        ...
                        return (
                          <button key={key} onClick={() => setCFilter(key)} style={{
                            ...
                            background: active ? (cm?.light ?? "#eef2ff") : "#fff",
                            color: active ? (cm?.text ?? "#4338ca") : "#64748b",
              ...
              {/* Empty filter */}
              {shown.length === 0 && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#94a3b8", padding: "40px 0" }}>
                  <div style={{ fontSize: 36, marginBottom: 10 }}>🔭</div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#334155", marginBottom: 4 }}>No matches</div>
                  <div style={{ fontSize: 12 }}>Try adjusting filters or clearing the search box.</div>
                </div>
              )}
              ...
              <div
                ...
                style={{ overflowY: "auto", flex: 1, paddingBottom: 20, alignContent: "start" }}
              >
                {mode === "closing"    && shown.map((r, i) => <CRCard    key={i} row={r as CRRow}  idx={i} listView={listView} />)}
                {mode === "allotments" && shown.map((r, i) => <AllotCard key={i} row={r as CSVRow} idx={i} listView={listView} />)}
