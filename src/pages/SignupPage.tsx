...
// import { getStoredUTMs } from '../utils/utmTracker';

// /**
//  * Signup Page Component
//  * Handles user registration with comprehensive form
//  * API Integration: POST /api/auth/signup/
//  * Google Sheets Integration: Stores data in Google Sheets simultaneously
//  */
...
//   // const GOOGLE_SHEETS_URL = "https://script.google.com/a/macros/believersdestination.com/s/AKfycbx03PXfHB1hUQI03AXYIk_HkdFiM5__3EZP29ch1C4fnyxHvDzEdFiUWnnk8jNVv3UO/exec";
...
//   //     // But the data will still be saved if the URL is correct
//   //   } catch (error) {
//   //     console.error("Error sending to Google Sheets:", error);
//   //     // Don't throw error - we don't want to block signup if Sheets fails
//   //   }
//   // };
...
//    * API Integration: POST /api/auth/signup/
//    * Also sends data to Google Sheets
//    */
...
//     //   // Send to your backend API (existing functionality)
...
//   await signup(fullPayload);         // ← now sends everything to Django
//   trackSignup();
//   navigate('/dashboard');
...
const SignupPage: React.FC = () => {
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    neetRank: "",        // ✅ FIXED: consistent camelCase everywhere
    category: "",
    state: "",
  });
  const [showPassword, setShowPassword] = useState(false);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fullPayload = {
        ...
        phone:        signupData.phone,
        password:     signupData.password,
        neetRank:     signupData.neetRank,   // ✅ AuthContext passes this → api.ts maps to neet_rank
        category:     signupData.category,
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-blue-50 to-indigo-50 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
          {/* Header */}
          <div className="text-center mb-8">
            <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-lg overflow-hidden mx-auto mb-3">
              <img src="/media/logo.png" alt="BD Logo" className="w-10 h-10 object-contain" />
            </div>
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">
              Join BelieversConsultancy
            </h1>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* NEET Rank — FIXED: name="neetRank" matches state key */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">NEET Rank (Optional)</label>
                <div className="relative">
                  <Award className="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" />
                  <input
                    type="text"
