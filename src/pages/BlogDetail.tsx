...
const BlogDetail: React.FC = () => {
  ...
  const hasFAQs =
    ...
    faqSections.some((s) => s.faqs && s.faqs.length > 0);

  const faqSchema = hasFAQs
    ? {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        mainEntity: faqSections.flatMap(
          (section) =>
  ...
  const renderSection = (section: Section) => {
    ...
    switch (section.type) {
      ...
      case "info-box": {
        ...
        return (
          <div
            className={`${boxColors[section.boxType || "info"]} border-l-4 p-4 rounded-r-lg mb-6`}
          >
            <div className="flex">
              <AlertCircle
                className={`w-5 h-5 ${iconColors[section.boxType || "info"]} mr-2 flex-shrink-0 mt-0.5`}
              />
                ...
                >
                  ...
                  <ul className="space-y-2">
                    {card.items.map((item, idx) => (
                      <li
                        key={idx}
                        className={`flex items-start ${colors.text} text-sm`}
                      >
  ...
  return (
    <>
      ...
      />
      {articleSchema && (
        <JsonLd data={articleSchema} id={`article-${post.slug}`} />
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <main className="lg:col-span-3">
              ...
              <div
                ref={contentRef}
                className="bg-white rounded-3xl shadow-lg p-6 lg:p-8 mb-8"
              >
                      ...
                      >
                        <div className="bg-white rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-200">
                          ...
                          <div className="p-4 bg-white">
                            <span
                              className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${relatedPost.category.color} mb-2`}
                            >
